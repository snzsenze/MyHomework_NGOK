    ...
    for (unsigned int i = 0; i < size - 1; i++) {
        for (unsigned int j = 0; j < size - i - 1; j++) {
            ...
            if (array[j] > array[j + 1]) {
                ...
                array[j] = array[j + 1]; // элемент будет равен другому тб(было
                                         // 10 2\ стало 2 10)
                array[j + 1] = tmp; // а след элемент будет равен тому что мы
                                    // записали на 2ст выше!(tmp = array[j];)
...
int binary_search(int array[], int size, int number)
{
    ...
    while (1) {
        if (number >
            array[mid_i]) { // если число которрое мы ввели больше центра то
            min_i = mid_i; // минимальное будет равно центральному
            mid_i =
                (min_i + max_i) / 2; // и новый мид будет равен (если сложить
            ...
            mid_i = (max_i + mid_i - 1) / 2; // центр будет равен (если сложить
                                             // максимальное и центр-1) и / 2
        }
        if ((mid_i == 0 || mid_i == size - 1) &&
            number != array[mid_i]) { // если наш мид == 0 или мид == размеру
                                      // масива) и число которое мы ввели не
